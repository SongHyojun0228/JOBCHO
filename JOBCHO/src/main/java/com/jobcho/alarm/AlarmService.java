...
public class AlarmService {
    ...
    public List<AlarmDTO> getAlarmsByUserId(int userId) {
    	 List<Alarms> alarms = alarmRepository.findByUserId(userId);
        ...
        return alarms.stream().map(alarm -> {
            ...
            if (alarm.getMention() != null) {
            	String senderName = userRepository.getById(alarm.getMention().getSenderId()).getUserName();
                dto.setMentionSenderName(senderName);
            }
            return dto;
        }).collect(Collectors.toList());
    }
    
    // 🌿 각 워크스페이스에 해당하는 알람 개수를 저장하는 map을 리턴하는 함수 
